...
import com.bigzhao.xml2axml.chunks.*;
...
public class Encoder {
    ...
    public byte[] encodeFile(Context context,byte[] in) throws XmlPullParserException, IOException {
        XmlPullParserFactory f=XmlPullParserFactory.newInstance();
        f.setFeature(XmlPullParser.FEATURE_PROCESS_NAMESPACES,true);
        XmlPullParser p=f.newPullParser();
        p.setInput(new ByteArrayInputStream(in),"UTF-8");
        return encode(context,p);
    }
    ...
    public byte[] encode(Context context,XmlPullParser p) throws XmlPullParserException, IOException {
        if (context==null) context=Main.thiz;
