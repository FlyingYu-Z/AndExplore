...
public class StringBlock implements IAXMLSerialize
{
		...
		if (mStringsCount > 0)
		{
			...
			for (int i =0; i < mStringsCount ; i++)
			{
				if ((mEncoder & 0x100) != 0)
				{
					...
					offset += varint[1];
					int length = varint[0];
					mStrings.add(i,new String(rawStrings,offset, length, Charset.forName(BASE128.decode("KYnYPK9d1g6ki50IhIT7JQ=="))));
	...
	public void write(IntWriter writer) throws IOException
	{
		...
		if (mStrings != null)
		{
			...
			{
				byte[] raw = s.getBytes(BASE128.decode("s9etNuAZUmSNdWcu/zVhpw=="));
...
//			}
	...
	public void prepare() throws IOException
	{
		...
		if (mStrings != null)
		{
			...
			for (int i =0; i < mStrings.size(); i++)
			{
				...
				try
				{
					size += 2 + mStrings.get(i).getBytes(BASE128.decode("s9etNuAZUmSNdWcu/zVhpw==")).length + 2;
				}
	...
	public static class Style
	{
		...
		public static Style parse(int[] muti_triplet) throws IOException
		{
			...
			{
				throw new IOException(BASE128.decode("gqhh8HszhYepOYK+Bd4p63f8GTCCUamyxvlhDkGBlfs="));
			}
	...
	{
		return BASE128.decode("mqyRe/WesewHegXP8VCoRg==") +
			BASE128.decode("YomIJCtoQ0CjBQ3VxgssBA==") + mChunkSize +
			BASE128.decode("vn0pEHVyJl3J9O2Oj4+64hc6851N1Lg/CkAIgUvVwng=") + mStringsCount +
			BASE128.decode("hGWOaj1gDVMM7s6tH3HgOw==") + mStylesCount +
			BASE128.decode("q1qjvL8bHUqcN+E/YZlmXA==") + mEncoder +
			BASE128.decode("lZ55ctk/BTJYZK38Q7A9MCTnkSJNWIcLY/KvL5lJdDQ=") + mStrBlockOffset +
			BASE128.decode("R8WGhRYjpBAZrddXqC5fPyTnkSJNWIcLY/KvL5lJdDQ=") + mStyBlockOffset +
			BASE128.decode("z7Imo9beyL/YS1XOx7BAbhc6851N1Lg/CkAIgUvVwng=") + Arrays.toString(mPerStrOffset) +
			BASE128.decode("arw3014107lRej/DLPe0JBc6851N1Lg/CkAIgUvVwng=") + Arrays.toString(mPerStyOffset) +
			BASE128.decode("r73BFrk/85PeR7lobsZ97A==") + mStrings +
			BASE128.decode("cmuRqPDDWxUvHnIfvRHAKw==") + mStyles +
			'}';
	}
}
