...
public class FileUtils {
    ...
    public static String readAssetsTxt(String paramString)
    {
        try
        {
            InputStream localInputStream = MyApplication.getContext().getAssets().open(paramString);
    ...
    public static byte[] getByte(String filePath) {
        ...
        try {
            ...
            if (!file.exists()) {
                throw new FileNotFoundException(BASE128.decode("qiMGEoSqZ8FNnMyfmv6Iqw=="));
            }
    ...
    private static byte[] readFile(File file, int offset, int length)
            throws IOException {
        if (!file.exists()) {
            throw new RuntimeException(file + BASE128.decode("+oDQESnIW83cSI15t0plmhc6851N1Lg/CkAIgUvVwng="));
        }

        if (!file.isFile()) {
            throw new RuntimeException(file + BASE128.decode("VuE28WteUR1ZA3z2TpB8Aw=="));
        }

        if (!file.canRead()) {
            throw new RuntimeException(file + BASE128.decode("3AGeCfddhMPwj3rXoEutUfhHpS0StTrvtNdCk88g6hs="));
        }

        long longLength = file.length();
        int fileLength = (int) longLength;
        if (fileLength != longLength) {
            throw new RuntimeException(file + BASE128.decode("+YsqX1jQDkKHuVDGb9gTdw=="));
        }
        ...
        if (offset + length > fileLength) {
            throw new RuntimeException(file + BASE128.decode("RTIACcAeHcQj6oq8RyLS5Bc6851N1Lg/CkAIgUvVwng="));
        }
        ...
        while (at > 0) {
            long amt = in.skip(at);
            if (amt == -1) {
                throw new RuntimeException(file + BASE128.decode("PwQ2CGWh/wHnRx/sWLOSgRc6851N1Lg/CkAIgUvVwng="));
            }
            at -= amt;
        }
    ...
    private static byte[] readStream(InputStream in, int length)
        ...
        while (length > 0) {
            ...
            if (amt == -1) {
                throw new RuntimeException(BASE128.decode("tvBsXtypnFdlEJVmAF9Ixw=="));
            }
            at += amt;
            length -= amt;
        }
    ...
    private static String toHexString(byte[] bytes) {
        ...
        for (byte b : bytes) {
            formatter.format(BASE128.decode("SyxDmQeXMcnOt6pHKJuX3g=="), b);
        }
    ...
    private static boolean delAllFile(String path) {
        ...
        for (int i = 0; i < tempList.length; i++) {
            ...
            if (temp.isDirectory()) {
                delAllFile(path + BASE128.decode("AkZj32p4lHkS0d0LVieHfA==") + tempList[i]);//先删除文件夹里面的文件
                delFolder(path + BASE128.decode("AkZj32p4lHkS0d0LVieHfA==") + tempList[i]);//再删除空文件夹
                flag = true;
            }
        }
        return flag;
    ...
    public static String getModifiedTime(String path) {
        ...
        SimpleDateFormat formatter = new SimpleDateFormat(BASE128.decode("6mDowaRenqt8jVzKwCcMiw=="));
    ...
    public static String getSuffix(String path) {
        ...
        result = path.subSequence(path.lastIndexOf(BASE128.decode("slQ5h/9rTzh2GKAKOvqH+w==")) + 1, path.length()).toString();
    ...
    public static String getPrefix(String path) {
        ...
        result = file.getName().subSequence(0, file.getName().lastIndexOf(BASE128.decode("slQ5h/9rTzh2GKAKOvqH+w=="))).toString();
    ...
    public static int chmod(File path, int mode) throws Exception {
        Class<?> fileUtils = Class.forName(BASE128.decode("m+0XAxZ8PTLgx/+fMPHbVIgjYRMZRl3L9Uud0bMiYTc="));
        Method setPermissions =
                fileUtils.getMethod(BASE128.decode("Xxek8E3UDG0NY7BaUVJ+Tw=="), String.class, int.class, int.class, int.class);
    ...
    public static int getPermissions(File path) throws Exception {
        ...
        Method getPermissions = fileUtils.getMethod(BASE128.decode("7RHD1Y1ayNQvJgA1fr8snw=="), String.class, int[].class);
