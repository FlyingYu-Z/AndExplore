package com.beingyi.app.AE.view;
...
public class CodeEditor extends FreeScrollingTextField {
    ...
    public CodeEditor(Context context, AttributeSet attributes) {
    ...
    public void init(Context context) {
        ...
        /**
        File df = new File(fontDir, BASE128.decode("uibKT3RQDlrqrK6bY2gTdQ=="));
        if (df.exists())
            setTypeface(Typeface.createFromFile(df));
        File bf = new File(fontDir, BASE128.decode("0EHBiYBaRc63rWKltJ4sQg=="));
        if (bf.exists())
            setBoldTypeface(Typeface.createFromFile(bf));
        File tf = new File(fontDir, BASE128.decode("4mShHw9GItWqzbZEEy8vvg=="));
        if (tf.exists())
        ...
        new AsyncTask<String, String, String[]>(){
            ...
            protected String[] doInBackground(String... strings) {
                ...
                for (int i = 0; i < cls.length; i++) {
                    String cl = cls[i];
                    int d = cl.lastIndexOf(BASE128.decode("hKNXm6a3UeZn9/SGePY07A=="));
                    if(d<0)
                      d = cl.lastIndexOf(BASE128.decode("slQ5h/9rTzh2GKAKOvqH+w=="));
                    if(d>0)
    ...
    public void addPackage(String pkg, String[] names) {
        Language lang = (LanguageLua) Lexer.getLanguage();
    ...
    public void startGotoMode() {
        ...
        startActionMode(new ActionMode.Callback() {
            ...
            public boolean onCreateActionMode(ActionMode mode, Menu menu) {
                // TODO: Implement this method
                mode.setTitle(BASE128.decode("IAiXSAeCvxKLs5ymqToIrw=="));
                ...
                menu.add(0, 1, 0, BASE128.decode("FzrznU3UuD8KQAiBS9XCeA==")).setActionView(edit);
    ...
    public void startFindMode() {
        ...
        startActionMode(new ActionMode.Callback() {
            ...
            public boolean onCreateActionMode(ActionMode mode, Menu menu) {
                // TODO: Implement this method
                mode.setTitle(BASE128.decode("VdLYmHRcegDmI1eZhwzBxA=="));
                mode.setSubtitle(null);
            ...
            private void findNext() {
                ...
                if (idx == -1) {
                    selectText(false);
                    Toast.makeText(mContext, BASE128.decode("sSstqEsIaVS7+FuQ3sjKSg=="), Toast.LENGTH_SHORT).show();
                    idx = 0;
                    return;
                }
    ...
    public boolean findUp(String keyword) {
        ...
        if (kw.isEmpty()) {
            selectText(false);
            Toast.makeText(mContext, BASE128.decode("HNnatU6iI8/qm7+AF+9MV4XbRZa6A90QKICkNklNrDo="), Toast.LENGTH_SHORT).show();
            return false;
        }
        idx = finder.findBackwards(getText(), kw, idx, getText().length(), false, false);
    ...
    public boolean findDown(String keyword) {
        if (!keyword.equals(mKeyword)) {
            mKeyword = keyword;
            idx = 0;
        }
