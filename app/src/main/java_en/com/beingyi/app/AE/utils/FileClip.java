                ...
                new Thread() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            zipTree.saveFile();
                            activity.showToast(BASE128.decode("AdKvJpl53b15YZn/vp1PLQ=="));
    ...
    public void putToDir(FilesAdapter adapter, final String dir, final String Name) {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            for (File file : fileList) {
                                ...
                                progres.setProgress(fileList.indexOf(file), fileList.size());
                                copy(file.getAbsolutePath(), file.getAbsolutePath().replace(cuFilePath, dir + Name + BASE128.decode("AkZj32p4lHkS0d0LVieHfA==")));
        ...
        if (cuType == 2) {
            ...
            new Thread() {
                @Override
                public void run() {
                    progres.setLabel(BASE128.decode("/140DO9vaV0mIUU8yWk7CQ=="));
                    progres.setNoProgress();
                    progres.show();
                    try {
                        zipTree.extract(cuEntry, dir);

                        activity.showToast(BASE128.decode("tWEJt3UTwEbXQDrbEx9N9Q=="));
                        activity.refreshList();
