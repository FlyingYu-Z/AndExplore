            ...
            public boolean onMenuItemClick(MenuItem item) {
                ...
                switch (id) {
                    case R.id.action_dex_edit:
                        ...
                        intent.putExtra(BASE128.decode("M13MUqoK/AAi9GI5H/keSA=="), isInZip);
                        intent.putExtra(BASE128.decode("cr62CT1jqczIn9Pg6z6GZg=="), file.getAbsolutePath());
                        context.startActivity(intent);
                        break;

                    case R.id.action_dex_tojar: {
                        final File outFile = new File(new File(Path).getParent(), FileUtils.getPrefix(Path) + BASE128.decode("vzFwdS+fdBhnGRHH5dkFGg=="));
                    ...
                    case R.id.action_dex_toSmali: {
                        final File outDir = new File(new File(Path).getParent(), FileUtils.getPrefix(Path) + BASE128.decode("3BiZts67m0DtzniDAfnpJA=="));
                    ...
                    case R.id.action_dex_toJava: {
                        final File outDir = new File(new File(Path).getParent(), FileUtils.getPrefix(Path) + BASE128.decode("o4ibzvOskSj1QkrC9SMWJA=="));
                    ...
                    case R.id.action_dex_addDex:
                        new selectFile(context, window, FileUtils.getSDPath(), BASE128.decode("VgRrq1lBW0G/1ZLBdMGKtw=="), new SelectFileCallBack() {

                            @Override
                            public void onSelected(String selectedPath) {
                                final File outFile = new File(new File(Path).getParent(), FileUtils.getPrefix(Path) + BASE128.decode("EslyWQz1hVM3pyWFdRAFHA=="));
                                new getSavePath(context, window, outFile.getAbsolutePath(), new GetSavePathCallBack() {
            ...
            public void run() {
                ...
                try {
                    ...
                    for (int i = 0; i < dexBackedClassDef.size(); i++) {
                        ...
                        File smaliFile = new File(utils.DesPath(outDir) + (type.substring(1, type.length() - 1)) + BASE128.decode("ghAenFZGOGAjsymuAFSYzg=="));
            ...
            public void run() {
                ...
                try {
                    ...
                    for(int i=0;i<javaClasses.size();i++){
                        ...
                        File javaFile = new File(utils.DesPath(outDir) + (fullName.replace(BASE128.decode("slQ5h/9rTzh2GKAKOvqH+w=="),BASE128.decode("AkZj32p4lHkS0d0LVieHfA=="))) + BASE128.decode("heaA92H6uOb2iKdsR0Hp3g=="));
            ...
            public void run() {
                ...
                try {
                    ...
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            activity.showToast(BASE128.decode("IHQ8OlugQ4kMFeWApU1RCA=="));
                            adapter.refresh();
                            adapter.setItemHighLight(outPath);
                        }
                    });
