...
import com.beingyi.app.AE.axmleditor.utils.IOUtils;
...
public class AXMLDoc {
	...
	public void testSize() throws IOException{
		out.println(BASE128.decode("on016GYpBpa7tI4+rwzNjgne4vzK+xW/1bgqoWGg0sA=") + mStringBlock.getSize());
		mStringBlock.prepare();
		out.println(BASE128.decode("on016GYpBpa7tI4+rwzNjgne4vzK+xW/1bgqoWGg0sA=") + mStringBlock.getSize());
		
		out.println(BASE128.decode("EKCKgf1PnH3eM5LlcXVqMg==") + mResBlock.getSize());
		mResBlock.prepare();
		out.println(BASE128.decode("H4fPEu/Uj7xXmFStq4svrw==") + mResBlock.getSize());
		
		out.println(BASE128.decode("WZVMhzKZ7sQYg3NNlyeNUg==") + mXMLTree.getSize());
		mXMLTree.prepare();
		out.println(BASE128.decode("WZVMhzKZ7sQYg3NNlyeNUg==") + mXMLTree.getSize());
		
		out.println(BASE128.decode("jfHj+sL9KgMlKrULixsUUg==") + mDocSize);
		int base = 8;
		int size = base + mStringBlock.getSize() + mResBlock.getSize() + mXMLTree.getSize();
		out.println(BASE128.decode("jfHj+sL9KgMlKrULixsUUg==") + size);
	}
	
	public void print(){
		out.println(BASE128.decode("sFUc1unD8ZtRRJ1D45X5rA==") + mDocSize);
	...
	public void parse(InputStream is) throws Exception{
		...
		if(magicNum != MAGIC_NUMBER){
			throw new RuntimeException(BASE128.decode("XlhW4HjqFey+K7qmwxGbih0hUc79VzAs6HoMtblHEI4="));
		}
